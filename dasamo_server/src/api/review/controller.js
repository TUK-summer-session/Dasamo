...
exports.storeQuestion = async (req, res) => {
  ...
  const { memberId, isQuestionForQuestion, parentQuestion, detail } = req.body;
  const reviewId = req.params.reviewId;

  // 값이 undefined인 경우 null로 설정
  const sanitizedMemberId = memberId !== undefined ? memberId : null;
  const sanitizedIsQuestionForQuestion = isQuestionForQuestion !== undefined ? isQuestionForQuestion : false;
  const sanitizedParentQuestion = parentQuestion !== undefined ? parentQuestion : null;
  const sanitizedDetail = detail !== undefined ? detail : '';

  try {
    await repository.storeQuestions({
      memberId: sanitizedMemberId,
      reviewId,
      isQuestionForQuestion: sanitizedIsQuestionForQuestion,
      parentQuestion: sanitizedParentQuestion,
      detail: sanitizedDetail,
    });
    const response = {
      status: 200,
      message: "리뷰 댓글을 성공적으로 저장했습니다."
    };
    return res.send(response); // 응답을 보내고 함수를 종료합니다.
  } catch (error) {
    console.error("Query error:", error);
    const response = {
      status: 500,
    ...
    return res.status(500).send(response); // 에러 응답을 보내고 함수를 종료합니다.
  }
};
...
exports.scrap = async (req, res) => {
  ...
  try {
    const alreadyLiked = await repository.checkIfAlreadyScraped(
      memberId,
      feedId
    );
    if (alreadyLiked) {
